...
class EFLayer():
    ...
    def get_individual_layer_prob(self, x, params):  # q(bx;\btheta)
        # Compute q(bx;\btheta) = exp[\sum_i [\log r_i (x_i) +sum_j \theta_i,j *f_i^j (x) - A_i (\theta_i) ] ]
        # Input
    ...
    def get_individual_params(self, bias):
        # the parameters of exponential family distribution when the layer isn't connected with other layers
...
class CategoricalLayer(EFLayer):
    def __init__(self,
                 ...
                 *args, **kwargs):
        kwargs['name'] = name
        super(CategoricalLayer, self).__init__(**kwargs)
...
class EFRBM(RBM):
    def __init__(self,
        ...
        super(EFRBM, self).__init__(**kwargs)
    ...
    def _create_layer(self, layer_type, suf_stat_dim):
        if layer_type == 'binary':
            ...
            return ContinuousLayer(suf_stat_dim=2, trainable_sigma2=self.Gaussian_layer_trainable_sigmal2)
        return None

    def _init_params(self, x):
        self.h_ = self.h_init * np.random.randn(1, self.num_hidden, self.suf_stat_dim_hid)

        self.v_ = self.v_init * np.random.randn(1, self.num_visible,
                                                self.suf_stat_dim_vis)

        self.w_ = self.w_init * np.random.randn(self.num_visible,
                                                self.num_hidden,
                                                self.suf_stat_dim_vis,
                                                self.suf_stat_dim_hid)

        self.hgrad_inc_ = np.zeros([1, self.num_hidden, self.suf_stat_dim_hid])
        self.vgrad_inc_ = np.zeros([1, self.num_visible, self.suf_stat_dim_vis])
        self.wgrad_inc_ = np.zeros([self.num_visible,
                                    self.num_hidden,
                                    self.suf_stat_dim_vis,
                                    self.suf_stat_dim_hid])

        self.v_, self.w_ = self.visible_layer.init_params(self.v_, self.w_, layer=0)
        self.h_, self.w_ = self.hidden_layer.init_params(self.h_, self.w_, layer=1)
    ...
    def _fit_loop(self, x, y,
        ...
        while (self.epoch_ < self.num_epochs) and (not self.stop_training_):
            ...
            for batch_idx, (batch_start, batch_end) in enumerate(batches):
                ...
                params_hidden_cond = self.hidden_layer.get_cond_dist_params(hidden_input, self.h_)
                ...
                if self.hidden_layer.hidden_sparsity == True and self.sparse_weight > 0:
                    ...
                    hg, wg, prev_hid_act = self._hidden_sparsity(sufficient_stat_batch, prev_hid_act, hid_act)
                    pos_hgrad += hg
                    pos_wgrad += wg
                ...
                if self.learning_method == LEARNING_METHOD['cd']:
                    for icd in range(self.num_cd - 1):
                        ...
                        suf_stat_vsample, hidden_input, params_hidden_cond = \
                            self._gibbs_sampling(params_hidden_cond, sampling=CD_SAMPLING['hidden_visible'])
    ...
    def _initialize_grad(self):
        pos_hgrad = np.zeros(self.h_.shape)
        pos_vgrad = np.zeros(self.v_.shape)
        pos_wgrad = np.zeros(self.w_.shape)
    ...
    def _forward(self, sufficient_stat_visible, **kwargs):
        ...
        coff = self.visible_layer.get_weight_coff(self.v_)
        for i in range(self.suf_stat_dim_hid):
            S = np.zeros([num_data, self.num_hidden])
            for j in range(self.suf_stat_dim_vis):
                w_coff = np.multiply(self.w_[:, :, j, i], coff[:, :, j].T)
                S += np.matmul(sufficient_stat_visible[:, :, j], w_coff)  # #samples x # hidden units
    ...
    def _backward(self, sufficient_stat_hidden, **kwargs):
        ...
        coff = self.hidden_layer.get_weight_coff(self.h_)
        for i in range(self.suf_stat_dim_vis):
            S = np.zeros([num_data, self.num_visible])
            for j in range(self.suf_stat_dim_hid):
                w_coff = np.multiply(self.w_[:, :, i, j].T, coff[:, :, j].T)
                S += np.matmul(sufficient_stat_hidden[:, :, j], w_coff)  # #samples x # visible units
            layer_input[:, :, i] = S
    ...
    def _get_positive_grad(self, sufficient_stat_visible, visible_input,
        ...
        num_data = sufficient_stat_visible.shape[0]
        vis_energy_expect =self.visible_layer.get_visible_energy_deri_expectation(sufficient_stat_visible,
                                                                                  bias = self.v_,
        ...
        hid_energy_expect = self.hidden_layer.get_hidden_energy_deri_expectation(sufficient_stat_visible=None,
        ...
        wg = np.zeros(self.w_.shape)

        # 1 x # visible units x  # visible sufficient stat dim
        visible_weight_coff = self.visible_layer.get_weight_coff(bias=self.v_)
        # 1 x # hidden units x  # hidden sufficient stat dim
        hidden_weight_coff = self.hidden_layer.get_weight_coff(bias=self.h_)

        expectation_hidden = self.hidden_layer.hid_suf_stat_posterior_expectation(params_cond=params_hidden_cond)
        for i in range(self.suf_stat_dim_vis):
            for j in range(self.suf_stat_dim_hid):
    ...
    def _gibbs_sampling(self, params_hidden_cond, sampling=CD_SAMPLING['hidden_visible']):
        if sampling == CD_SAMPLING['hidden']:
            ...
            params_visible_cond = self.visible_layer.get_cond_dist_params(visible_input, self.v_)
    ...
    def get_reconstruction(self, x):
        ...
        params_visible_cond = self.visible_layer.get_cond_dist_params(visible_input,self.v_)
        vis_suf_stat_mean = self.visible_layer.hid_suf_stat_posterior_expectation(params_visible_cond)
    ...
    def get_free_energy(self, x):
        ...
        num_data = x.shape[0]
        indi_params_visible = self.visible_layer.get_individual_params(self.v_)
        indi_params_hidden = self.hidden_layer.get_individual_params(self.h_)
    ...
    def _hidden_sparsity(self, sufficient_stat_x, prev_hid_act, hid_act):
        ...
        for i in range(self.suf_stat_dim_vis):
            for j in range(self.suf_stat_dim_hid):
                wg[:, :, i, j] = self.sparse_weight * sufficient_stat_x[:, :, i].T.dot(sparse_grad[:, :, j]) / sufficient_stat_x.shape[0]
