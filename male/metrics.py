...
def init_inception():
    ...
    with tf.Session(graph=inception_graph, config=tf_config) as sess:
...
def inception_score(inception_model, inception_graph, images, splits=10):
    import tensorflow as tf
    from .backend import tensorflow_backend as tf_backend
    ...
    with tf.Session(graph=inception_graph, config=tf_config) as sess:
        ...
        for i in range(n_batches):
            ...
            pred = sess.run(inception_model, {'ExpandDims:0': inp})
            preds.append(pred)
