...
def load_pretrain_of_Ruslan(folder=None):
    if folder is None:
        folder = 'g:/hungv/Dropbox/WithHung_Experiment/experiments/pycs/pycs/app/anom/DBM_ruslan/code_DBM'
...
def test_dbm_mnist():
    ...
    if data_type == 0:
        print('Loading our MNIST data\n')
        x_train, y_train, x_test, y_test = load_mnist()
        # x = np.vstack([x_train, x_test])
        # y = np.concatenate([y_train, y_test])

    elif data_type == 1:
        print('Loading Ruslan'' MNIST data\n')
    ...
    if pretrained_params is None:
        pretrain_method = 'OurRBMs'
    else:
        pretrain_method = 'RuslanModel'

    model_path = "%s/DBM/%s/numtrain%d_pretrain-%s" % (
    temp_folder, method, x_train.shape[0], pretrain_method)
    ...
    model = DBM(
        ...
        layer_types=['binary', 'binary', 'binary', 'binary'],
