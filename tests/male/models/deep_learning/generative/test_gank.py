...
def test_gank_mnist(block_figure_on_end=False):
    print("========== Test GANK on MNIST data ==========")
...
@pytest.mark.skipif('tensorflow' not in sys.modules, reason="requires tensorflow library")
def test_gank_cifar10(block_figure_on_end=False):
    print("========== Test GANK on CIFAR10 data ==========")
    ...
    model = GANK(model_name="GANK_CIFAR10",
                 ...
                 metrics=['d_loss', 'g_loss'],
                 callbacks=[loss_display, sample_display],
                 num_epochs=500,  # set to 100 for a full run
                 random_state=random_seed(),
                 verbose=1)
